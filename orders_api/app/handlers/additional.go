...
func UpdateAdditional(c *fiber.Ctx) error {
	// Obter o ID do item adicional dos parÃ¢metros da rota
	...
	// Analisar a solicitaÃ§Ã£o do corpo para obter os dados de atualizaÃ§Ã£o
	var request dto.AdditionalRequest
	if err := c.BodyParser(&request); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Failed to parse request body"})

	}
	// Atualizar os campos relevantes do item adicional com os dados fornecidos na solicitaÃ§Ã£o
	...
	// Salas alteraÃ§Ãµes no banco de dados
...
func CreateProductToAdditional(c *fiber.Ctx) error {
	...
	if result.Error != nil {
		// Se houver um erro, o produto nÃ£o existe
	...
	if result.RowsAffected > 0 {
		// O relacionamento jÃ¡ existe, entÃ£o vamos removÃª-lo
