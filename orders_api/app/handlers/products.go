...
func GetByEstablishmentId(c *fiber.Ctx) error {
	...
	models.DB.Where(&models.Product{EstablishmentID: uint(establishmentId)}).Preload("Additional").Find(&product).Preload("Categories").Find(&product)
...
func CreateMultProducts(c *fiber.Ctx) error {
	...
	for _, request := range requests {
		// Verificar se o estabelecimento existe (se necessÃ¡rio)
		...
		// Criar o produto e adicionar Ã  slice de produtos criados
		models.DB.Create(&product)
...
func GetByEstablishmentIdWithRelations(c *fiber.Ctx) error {
	...
	models.DB.Where(&models.Category{EstablishmentID: uint(establishmentId)}).Find(&categories)
	for _, category := range categories {

		var products []models.Product
		models.DB.Model(&category).Association("Products").Find(&products)
		categoriesWithProducts = append(categoriesWithProducts, models.CategoryProducts{Category: category})
